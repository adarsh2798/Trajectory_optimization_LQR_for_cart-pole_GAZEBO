...
from balance_optimal_input_trajOPT import *
...
reset_simulation()
pause_simulation()
#rospy.sleep(5.0)  # Pause the simulation for 3 seconds
states = np.zeros(4)
...
u_d_balance=1*compute_input_balance(np.array([0.27406165 , 0.50578686 , 0.00418623 , 3.00087477])*1)
#subprocess.run(['rostopic', 'pub', '/gazebo/unpause', 'std_msgs/Empty', '-1'])
    ...
    while not rospy.is_shutdown():
        ...
        if TOP_CHECK==0:
            ...
            if entering_TOP==0:
                ...
                entering_TOP=1 # already entered top
                start=time.time()
                
            else:
                t_curr=time.time()-start
                u_opt=get_interpolated_u_balance(t_curr,u_d_balance)
