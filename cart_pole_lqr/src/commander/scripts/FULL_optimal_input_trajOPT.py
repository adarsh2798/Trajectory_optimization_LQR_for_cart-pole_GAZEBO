...
g=9.8
T=10 #final time 
N=50 # no. of collocation points
t_d=np.linspace(0,T,N) # discrete time steps
d_bar=0.5# final cart x-pos after top reached
tau_max=10 # max input to base joint
d_max=0.7# max cart x- deviation allowed
...
def get_interpolated_u(t_curr,u_d):
     if(t_curr>T):
          return 0
     if(t_curr==0):
          return 0
